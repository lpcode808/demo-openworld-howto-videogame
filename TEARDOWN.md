...
**Where it lives:** §10 BOOT, lines 1230–1290, then §8 LOOP, lines 1137–1167. Read them in that
...
run once when the page opens. Follow them top to bottom: two checks on the data (1240–1274), the
...
Now go to `frame`, lines 1148–1167. Nineteen lines. Read each one and say in words what it does.
...
**Where it lives:** §2 DATA, lines 228–483. The maps themselves are lines 257–313; the
overworld's rows are lines 260–291.
...
Now find `tileTypes`, lines 239–248. This table is the legend: what each character means, what
...
**Where it lives:** §3 STATE, lines 486–536. The object itself is `state`, lines 500–535.
...
Two things outside `state` do change while the game runs, and the comment at lines 496–499 names
...
**Where it lives:** §6 COLLIDE, lines 802–852. `canWalkTo` is lines 839–851; `tileAt` is
lines 811–816.
...
**Where it lives:** §7 RENDER, lines 855–1134, and three lines of §8 LOOP, lines 1157–1165.
...
Now look at how the file enforces it. Line 1157 turns `state` into a string before drawing. Line
...
**Break it on purpose:** inside `drawNotice`, after line 1114, add a new line
...
`state.flags.bridgeBuilt` instead. That flag is set to `true` in `giveItemToNpc` (line 795) when
...
and both `drawItems` (line 967) and `pickUpItemUnderPlayer` (line 717) check `pickedUpItems`, so
...
(line 795) set `state.flags.bridgeBuilt = true`. RENDER noticed on the next frame because it

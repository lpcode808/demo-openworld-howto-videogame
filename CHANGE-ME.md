...
**Where:** §1 CONFIG, line 75 (`playerStepsPerSecond`)
...
**Where:** §1 CONFIG, lines 98–122 (`colors`)
**You should see:** Every tile of that terrain type, on both maps, changes color.

## 3. Widen the map by typing

**Do:** Add one more character to the end of every row of the overworld, keeping all rows equal.
**Where:** §2 DATA, lines 192–221 (`maps.overworld.rows`)
...
**Where:** §2 DATA, lines 268–273 (`items`)
...
**Where:** §2 DATA, lines 289–399 (`npcs`)
**You should see:** A new colored square with a name floating above it, and a speech box when
you face it and press Space.

## 6. Make an NPC move

**Do:** Make one NPC wander instead of standing still. Its position in the table below is
`const`, so it can't be reassigned there — think about which section is allowed to change a
value over time, and where a value that changes over time is supposed to live.
**Where:** §2 DATA, lines 289–399 (`npcs`)
**Where:** §3 STATE, lines 431–466 (`state`)
**Where:** §5 UPDATE, lines 557–568 (`update`)
...
map row.
**Where:** §1 CONFIG, lines 98–122 (`colors`)
**Where:** §2 DATA, lines 170–180 (`tileTypes`)
**Where:** §2 DATA, lines 192–221 (`maps.overworld.rows`)
...
**Where:** §2 DATA, lines 188–244 (`maps`)
**Where:** §2 DATA, lines 192–221 (`maps.overworld.rows`)
**Where:** §2 DATA, lines 248–257 (`doors`)
...
**Where:** §2 DATA, lines 277–281 (`quests`)
**Where:** §2 DATA, lines 289–399 (`npcs`)
**Where:** §5 UPDATE, lines 676–691 (`updateTalking`)
**Where:** §5 UPDATE, lines 695–722 (`updateDialogue`)
**Where:** §5 UPDATE, lines 726–730 (`giveItemToNpc`)
...
**Where:** §3 STATE, lines 451–455 (`state.flags`)
**Where:** §2 DATA, lines 277–281 (`quests`)
**Where:** §5 UPDATE, lines 557–568 (`update`)
**Where:** §7 RENDER, lines 950–989 (`drawHud`)
